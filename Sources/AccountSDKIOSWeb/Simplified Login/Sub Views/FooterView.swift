...
class FooterView: UIStackView {
    ...
    private lazy var schibstedIconImageView: UIImageView = {
        ...
        let image: UIImage = UIImage(named: viewModel.schibstedLogoName, in: Bundle.resourceBundle(for: FooterView.self), compatibleWith: nil) ?? UIImage()
    ...
    private func getRoundedImageView(name: String) -> UIImageView {
        ...
        let image  = UIImage(named: name, in: Bundle.resourceBundle(for: FooterView.self), compatibleWith: nil) ?? UIImage()
        view.image = image
