...
struct AuthenticatedURLSessionTests {
    ...
    @Test("dataTask should retry when the response status code is HTTP 401")
    func dataTaskRetryWhenUnauthorized() async throws {
        let urlSession = FakeURLSession()
        let keychainStorage = FakeKeychainStorage()
        let clientId = "a70ed9c041334b712c599a526"
        ...
        let userUUID = UUID().uuidString
        let userTokens = UserTokens.fake(userUUID: userUUID)
        let userSession = UserSession(userTokens: userTokens, updatedAt: Date())
    ...
    @Test("dataTask should retry when the access token has expired")
    func dataTaskRetryWhenExpired() async throws {
        let urlSession = FakeURLSession()
        let keychainStorage = FakeKeychainStorage()
        let clientId = "a70ed9c041334b712c599a526"

        let userUUID = UUID().uuidString
        let userTokens = UserTokens.fake(userUUID: userUUID, expiration: Date(timeIntervalSinceNow: -10))
        let userSession = UserSession(userTokens: userTokens, updatedAt: Date())
        ...
        let expectation = TestExpectation(expectedFulfillmentCount: 1)
